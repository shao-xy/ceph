...
{
  return out << '[' << dout_wrapper<CDir*>(dir) << " (" << cur_load << ',' << cur_epoch << ")/(" << predicted_load << ',' << predicted_epoch << ")]";
}
...
//#endif
      ...
      if (dir->get_inode()->get_global_depth() < g_conf->adsl_mds_predictor_debug_loadmat_dir_level) {
	 string s;
...
int dirfrag_load_pred_t::do_predict(Predictor * predictor)
{
  ...
  dout(20) << __func__ << " start " << *this << dendl;
...
#else
  if (dir->get_inode()->get_global_depth() < g_conf->adsl_mds_predictor_debug_result_dir_level) {
    dout(0) << __func__ << " PREDICTOR_DEBUG " << dout_wrapper<CDir*>(dir) << " After prediction, pred_load " << predicted << dendl;
  }
#endif
...
mds_load_t MDBalancer::get_load(utime_t now)
{
  ...
  if (mds->mdcache->get_root()) {
	...
	load.pred_auth += (*p)->pop_pred.meta_load();
      ...
      dout(15) << "MDBalancer::" << __func__ << " After add " << load.pred_auth << ", dir " << *(*p) << dendl;
    }
  } else {
...
{
  string pred_name = mds->mdsmap->get_predictor();
  if (!predictor.need_read_rados(pred_name)) {
    pred_code.assign("");
